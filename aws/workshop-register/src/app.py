"""Workshop pre-registration: POST form to DynamoDB (HTTP API + Lambda)."""
...
def _duplicate_location() -> str:
    return PUBLIC_SITE_ORIGIN + "/workshop-register.html?duplicate=1"
...
def handler(event, context):
    ...
    if participation_mode not in _PARTICIPATION_OK:
        return _bad_request("Please select in person or remote participation.")
    if bring_laptop not in ("yes", "no"):
        return _bad_request("Please indicate whether you will bring a laptop.")
    if ack_limited_seats != "yes":
        return _bad_request("Please acknowledge limited seats to continue.")
